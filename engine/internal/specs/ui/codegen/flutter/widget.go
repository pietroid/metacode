...
func writeWidget(comp model.UIComponent, symbols model.SymbolTable, c *catalog.Catalog, pagesDir, widgetsDir string, tmpl *template.Template) error {
	...
	vars := dart.UniqueStrings(comp.Variables)
...
func (r *renderer) renderCatalogWidget(comp model.UIComponent, sym catalog.Symbol, topLevel bool) string {
	...
	if topLevel {
		args = append(args, fmt.Sprintf("key: const Key(%s)", dart.DartStringLiteral(comp.Name)))
	}
